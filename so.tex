...
onde instruções de diferentes processos ou threads são executadas literalmente no mesmo instante de tempo físico, em núcleos físicos distintos.
...
O tamanho de cada quadro é de $1 \unit{Kbyte}$ (considerando $\unit{K} = 1024$), o que requer 10 bits de deslocamento ($2^{10} = 1024 \unit{bytes}$).
...
Uma trava (\textit{lock}) é um dos mecanismos utilizados para garantir a exclusão mútua,
permitindo que apenas uma thread ou processo acesse um recurso compartilhado ou região crítica por vez \parencite[cap.~2,6]{tanenbaum2016}.
