...
em núcleos físicos distintos \parencite[cap.~2]{tanenbaum2016}.
O gerenciamento adequado da concorrência e do paralelismo é um dos pilares modernos do design de sistemas operacionais.
...
e por quanto tempo \parencite[cap.~2]{tanenbaum2016}.
Diferentes ambientes exigem diferentes estratégias de escalonamento.
...
Em sistemas interativos, onde o tempo de resposta é crucial, utilizam-se o \textit{Round-Robin} (chave circular) e o Escalonamento por Prioridades.
Em sistemas de tempo real, o RMS (\textit{Rate Monotonic Scheduling}) e o EDF (\textit{Earliest Deadline First}) são os mais propícios.

Analisando as opções: a opção A cita FILO, o que seria uma pilha, inadequado para escalonamento justo;
...
a opção D sugere Round-Robin para tempo real (o que não garante cumprimento de prazos rígidos).
A alternativa correta é a E, pois o escalonamento por prioridades permite que tarefas interativas importantes tenham a resposta rápida necessária.
...
em quadros (\textit{frames}) de 1 Kbyte e endereçada por byte.
Os espaços de endereçamento dos processos são múltiplos de 1 Kbyte.
...
\begin{enumerate}[label=\Roman*.]
    ...
    \item A tradução do endereço lógico 01100000000110 para endereço físico causa exceção.
\end{enumerate}
...
O tamanho de cada quadro é de 1 Kbyte, o que requer 10 bits de deslocamento ($2^{10} = 1024$ bytes).
Observando a tabela, o número do quadro utiliza 4 bits (de \texttt{0000} a \texttt{1111}).
Portanto, o endereço físico completo é a junção dos 4 bits do quadro com os 10 bits de deslocamento, totalizando 14 bits, e não 13.

A afirmação II é verdadeira.
A paginação divide a memória em blocos de tamanho fixo, permitindo que qualquer quadro livre receba uma página de qualquer processo.
Isso elimina completamente a fragmentação externa \parencite[seç.~3.3]{tanenbaum2016}.

A afirmação III é verdadeira.
O endereço lógico \texttt{01100000000110} possui 14 bits. Como o deslocamento deve ter 10 bits (\texttt{0000000110}),
os 4 bits superiores indicam a página virtual: \texttt{0110}, que equivale ao número 6 em decimal.
Consultando a tabela para a página 6, vemos que o bit de validade (BV) é 0.
O acesso a uma página com bit de validade igual a zero gera uma interrupção no sistema operacional conhecida
como falta de página (ou \textit{page fault}), que é um tipo de exceção \parencite[seç.~3.3]{tanenbaum2016}.
...
utilizando a sequência de acessos às páginas 1-2-3-4-1-2-5-1-2-3-4-5 de um processo e memória inicialmente vazia.\\\\
...
no enunciado são parâmetros perfeitamente suficientes para realizar as simulações e constatar a diferença.
Portanto, a opção E é a correta.

\section{Threads (linhas de execução)}
Enquanto um processo tradicional possui um único fluxo de controle e um espaço de endereçamento independente,
as \textit{threads} permitem múltiplos fluxos de execução dentro do mesmo espaço de endereçamento de um processo.
Isso facilita o compartilhamento de dados, mas traz desafios profundos de sincronização,
sendo necessário o uso de mecanismos de exclusão mútua \parencite[cap.~2]{tanenbaum2016}.

\begin{question}{2014-20}
Um processo tem um ou mais fluxos de execução, normalmente denominados apenas por \textit{threads}.
\begin{quote}
	[Figura 1 apresenta 3 retângulos distintos identificados como Processo 1, Processo 2 e Processo 3,
	cada um contendo uma única \textit{thread} desenhada em seu próprio espaço de usuário isolado.]
	[Figura 2 apresenta 1 retângulo identificado como Processo,
	contendo três \textit{threads} juntas no mesmo espaço de usuário.]
\end{quote}
...
A afirmação IV é verdadeira, descrevendo o modelo clássico multiprogramado representado na figura 1.
...
Considera-se que as duas implementações são utilizadas por no máximo duas \textit{threads} e o código foi desenvolvido
...
O LockA implementa o método de alternância estrita.
\textcite[seç.~2.3.2]{tanenbaum2016} demonstram que esse método garante a exclusão mútua (Afirmação III verdadeira),
...
mas falha miseravelmente na garantia de exclusão mútua em sistemas com mais de um processador (Afirmação IV falsa).
...
a thread rodando no outro processador pode tranquilamente invadir a memória compartilhada \parencite[seç.~2.3.1]{tanenbaum2016}.
...
Esse código ilustra o cenário clássico de uma condição de corrida em memória compartilhada sem travas,
...
O único cenário impossível sob o modelo de consistência estrita é a impressão de ambos ``1'' e ``2'',
...
o que é logicamente paradoxal num fluxo de código linear, evidenciando que a resposta correta é a alternativa E.
